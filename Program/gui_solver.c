...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    const char CLASS_NAME[] = "ESI ex INI ex CERI LOGICAL FORMULAS SOLVER";
    ...
    mainWindow = CreateWindowEx(
        0,
        CLASS_NAME,
        "ESI ex INI ex CERI LOGICAL FORMULAS SOLVER",
        WS_OVERLAPPEDWINDOW,
    ...
    titleText = CreateWindow(
        "STATIC", "ESI ex INI ex CERI LOGICAL FORMULAS SOLVER",
...
void CreateMainMenu(HWND hwnd) {
    ...
    testButton = CreateWindow(
        "BUTTON", "Test an existing Formula",
        WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
        250, 180, 300, 45,
    ...
    showButton = CreateWindow(
        "BUTTON", "Show all available Formulas",
        WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
        250, 240, 300, 45,
...
void ShowFormulaCreation(HWND hwnd) {
    ...
    CreateWindow(
        "STATIC", "Enter the formula Name (e.g : TEST):",
        WS_VISIBLE | WS_CHILD,
        250, 140, 300, 25,
        hwnd, NULL,
        GetModuleHandle(NULL), NULL
    ...
    CreateWindow(
        "STATIC", "Number of Clauses included :",
        WS_VISIBLE | WS_CHILD,
        250, 220, 300, 25,
        hwnd, NULL,
        GetModuleHandle(NULL), NULL
...
void ShowCredits(HWND hwnd) {
    ...
    HWND titleText = CreateWindow(
        "STATIC", "LOGICAL FORMULAS SOLVER - 2CP",
        WS_VISIBLE | WS_CHILD | SS_CENTER,
        0, 80, 800, 40,
        hwnd, NULL,
        GetModuleHandle(NULL), NULL
    ...
    // Names with borders and white background
    HWND name1 = CreateWindow(
        "STATIC", "BOUKEHA Mohamed Akram",
    ...
    HWND name2 = CreateWindow(
        "STATIC", "KHARCHI Merouane Abderrahmane",
        WS_VISIBLE | WS_CHILD | SS_CENTER | WS_BORDER,
        200, 240, 400, 40,
        hwnd, NULL,
        GetModuleHandle(NULL), NULL
    ...
    // Set blue text color for each name using WM_CTLCOLORSTATIC
    HDC hdc;
    ...
    SendMessage(name4, WM_SETFONT, (WPARAM)hFontBig, TRUE);
    SendMessage(backButton, WM_SETFONT, (WPARAM)hFont, TRUE);

    // Ensure the window background is white
...
void ExecuteLogicSolver(HWND hwnd, const char* filename) {
    ...
    snprintf(command, sizeof(command), "logic_solver.exe \"%s\" 2>&1", filename);
    ...
    // Extract only the satisfiability result
    char result[100] = "";
    if (strstr(output, "UNSATISFIABLE") != NULL) {
        strcpy(result, "Result: Formula is UNSATISFIABLE");
    } else if (strstr(output, "SATISFIABLE") != NULL) {
        strcpy(result, "Result: Formula is SATISFIABLE");
    } else {
        strcpy(result, "Result: Unable to determine satisfiability");
    }
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_CTLCOLORSTATIC: {
            ...
            SetTextColor(hdcStatic, RGB(0, 0, 205));  // Medium blue
